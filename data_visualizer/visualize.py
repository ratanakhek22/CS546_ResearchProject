...
def parse_args():
    ...
    parser.add_argument("--run",     type=str, default=None,
                        help="Timestamp dir name. Defaults to latest run.")
    return parser.parse_args()

def resolve_run_dir(dataset, run):
    base = os.path.join("../results", dataset)
...
<div class="grid">
  ...
  <div class="card full">
    ...
    <table>
      <thead>
        <tr>
          <th>Strategy</th><th>Trials</th><th>Build time</th>
          <th>Preprocess time</th><th>Total time</th><th>Avg retrieval</th>
        </tr>
      </thead>
...
if __name__ == "__main__":
    args = parse_args()
    run_dir = resolve_run_dir(args.dataset, args.run)
    ...
    output_file = os.path.join("reports", f"{args.dataset}_{run_id}.html")
